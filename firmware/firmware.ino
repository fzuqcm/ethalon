...
// 65536
long gradient(long left_freq, long right_freq) {
  int step_size = (right_freq - left_freq) / STEP_NUM;
  if (step_size <= 1) {
    return left_freq + (STEP_NUM / 2) * step_size;
  }
...
//      Serial.println(a);
  ...
  return gradient(max_f - 4 * step_size, max_f + 2 * step_size);
}

int readAmpl() {
  return analogRead(AD8302_MAG);
}

int readPhas() {
  return analogRead(AD8302_PHASE);
}
...
void resonanceFrequencyOutput(long range)
{
    long rf = gradient(calib_freq - range, calib_freq + range);
    ...
    double result = -coeffs(1)/(2*coeffs(0)) * 16 + (double) rf;

    calib_freq = (int) result;
...
void loop()
{
  if ( (byteAtPort = Serial.available()) > 0) {
    ...
    bool isCalibration = strchr(buf, 'c') != NULL;
    if (isCalibration)
        calib_freq = DEFAULT_CALIB_FREQ;

    char* receivedCalibFreq = strchr(buf, 'm');
    bool isMeasurement = false;
    if (receivedCalibFreq != NULL) {
        if (strlen(receivedCalibFreq) > 1)
            calib_freq = atol(++receivedCalibFreq);

        isMeasurement = true;
    }

    if (isCalibration || isMeasurement) {
        resonanceFrequencyOutput(DEFAULT_RANGE);
        return;
    }

    bool isBinary = strchr(buf, '|') != NULL;
    // decode message
    while (isBinary ? (str = strtok_r(p, "|", &p)) != NULL : (str = strtok_r(p, ";", &p)) != NULL) { // delimiter is the semicolon
      // frequency start
      if (nn == 0) {
        freq_start = atol(str);
    ...
    if (message == 1) {
      ...
      long count = 0;
      pre_time = millis();
      int stepCount = (freq_stop - freq_start) / freq_step + 1;
      int magBuffer[stepCount];
      int phaseBuffer[stepCount];
      int *pMagBuffer = magBuffer;
      int *pPhaseBuffer = phaseBuffer;
      ...
      temperature = tempsensor.readTempC();
      if (isBinary) {
        Serial.write((uint8_t*)magBuffer, sizeof(magBuffer));
        Serial.write((uint8_t*)phaseBuffer, sizeof(phaseBuffer));
